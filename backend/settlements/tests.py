...
class TwoDigitSettlementTests(APITestCase):
    """A 2D period settles against the two-digit winning number."""

    def setUp(self):
        self.owner = User.objects.create_user(
            phone="+959320000001", password="pass12345", name="Owner", role="owner"
        )
        self.winner = User.objects.create_user(
            phone="+959320000002", password="pass12345", name="Winner", role="user"
        )

        now = timezone.now()
        self.period = ResultPeriod.objects.create(
            code="TWOD-SET-01",
            name="2D Settlement Period",
    ...
    def _receipt(self, number_code, amount, receipt_no):
        receipt = Receipt.objects.create(
            receipt_no=receipt_no,
            user=self.winner,
            result_period=self.period,
        ...
        ReceiptItem.objects.create(
            receipt=receipt, number_code=number_code, amount=Decimal(amount)
        )
        return receipt

    def test_two_digit_result_matches_and_pays(self):
        self._receipt("24", "100", "R-2D-WIN")
        self._receipt("99", "100", "R-2D-LOSE")

        batch = create_settlement_preview(self.period, "24", self.owner)

        self.assertEqual(batch.total_settlement, Decimal("9000.00"))  # 100 * 90
        self.assertEqual(batch.items.count(), 1)
        self.assertEqual(batch.items.first().user, self.winner)

    def test_three_digit_result_rejected_on_2d_period(self):
        with self.assertRaises(ValueError):
            create_settlement_preview(self.period, "124", self.owner)
